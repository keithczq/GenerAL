...
RGBDAggregate = collections.namedtuple('RGBDAggregate', ['rgb_image', 'depth_image', 'point_cloud_array', 'point_cloud_pcl', 'camera2world_tf_mat'])
...
class RGBDDataCollector:
    ...
    def tf_callback(self):
        while not rospy.is_shutdown() and self._still_running:
            if not self._read_lock.locked():
                now = rospy.Time(0)
                self._tf_listener.waitForTransform(self._camera_frame, self._world_frame, now, timeout=rospy.Duration(5))
    ...
    def ready(self):
        if self.capturing_tf_frames:
            return self._rgb_image is not None and self._depth_image is not None and self._pcl_array is not None and self._pcl_data is not None and self._camera2world_tf_mat is not None
        else:
...
def main():
    ...
    # init_service(rgb_image_topic, depth_image_topic, cloud_topic)  # Omit camera_frame and world_frame if you don't want tracking
